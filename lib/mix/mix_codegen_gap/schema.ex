defmodule Mix.MixCodegenGap.Schema do
  @moduledoc false
  ...
  def new(schema_name, schema_plural, cli_attrs, opts) do
    ...
    web_path = web_namespace && MixCodegenGap.Naming.underscore(web_namespace)
    ...
    %Schema{
      ...
      human_singular: MixCodegenGap.Naming.humanize(singular),
      human_plural: MixCodegenGap.Naming.humanize(schema_plural),
  ...
  defp partition_attrs_and_assocs(schema_module, attrs) do
    ...
    assocs =
      Enum.map(assocs, fn {key_id, {:references, source}} ->
        ...
        module = Module.concat(base ++ [MixCodegenGap.Naming.camelize(key)])
