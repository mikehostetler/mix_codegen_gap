defmodule Mix.MixCodegenGap.Context do
  @moduledoc false
  ...
  def new(context_name, %Schema{} = schema, opts) do
    ...
    base      = Module.concat([Mix.MixCodegenGap.context_base(ctx_app)])
    module    = Module.concat(base, context_name)
    alias     = Module.concat([module |> Module.split() |> List.last()])
    basedir   = MixCodegenGap.Naming.underscore(context_name)
    basename  = Path.basename(basedir)
    dir       = Mix.MixCodegenGap.context_lib_path(ctx_app, basedir)
    file      = dir <> ".ex"
    test_dir  = Mix.MixCodegenGap.context_test_path(ctx_app, basedir)
  ...
  defp web_module do
    ...
    cond do
      Mix.MixCodegenGap.context_app() != Mix.MixCodegenGap.otp_app() ->
        Module.concat([base])
