defmodule Mix.Tasks.Codegap.Gen.Cert do
  ...
  Generates a self-signed certificate for HTTPS testing.
      ...
      mix codegap.gen.cert
      mix codegap.gen.cert my-app my-app.local my-app.internal.example.com
  ...
  def run(all_args) do
    if Mix.Project.umbrella?() do
      Mix.raise("mix codegap.gen.cert can only be run inside an application directory")
    end
  ...
  defp print_shell_instructions(keyfile, certfile) do
    ...
    configuration in config/dev.exs:
      ...
      config #{inspect(app)}, #{inspect(Mix.MixCodegenGap.web_module(base))}.Endpoint,
        http: [port: 4000],
        https: [
          port: 4001,
          cipher_suite: :strong,
  ...
  defp rdn(common_name) do
    {:rdnSequence,
     [
       [attr(type: @organizationName, value: {:utf8String, "MixCodegenGap Framework"})],
