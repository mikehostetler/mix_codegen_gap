defmodule Mix.Tasks.Compile.MixCodegenGap do
  use Mix.Task
  @recursive true

  @moduledoc """
  Compiles MixCodegenGap source files that support code reloading.
  """

  @doc false
  def run(_args) do
    {:ok, _} = Application.ensure_all_started(:mix_codegen_gap)
  ...
  defp modules_for_recompilation(modules) do
    Stream.filter modules, fn mod ->
      Code.ensure_loaded?(mod) and
        function_exported?(mod, :__mix_codegen_gap_recompile__?, 0) and
        mod.__mix_codegen_gap_recompile__?
    end
  end
