defmodule Mix.Tasks.Codegap.Routes do
  use Mix.Task
  alias MixCodegenGap.Router.ConsoleFormatter
  ...
  Prints all routes for the default or a given router.
      ...
      $ mix codegap.routes
      $ mix codegap.routes MyApp.AnotherRouter
  ...
  @doc false
  def run(args, base \\ Mix.MixCodegenGap.base()) do
  ...
  defp router(nil, base) do
    if Mix.Project.umbrella?() do
      Mix.raise """
      umbrella applications require an explicit router to be given to codegap.routes, for example:

          $ mix codegap.routes MyAppWeb.Router
      """
    end
    ...
    An explicit router module may be given to codegap.routes, for example:
