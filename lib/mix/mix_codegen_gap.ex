defmodule Mix.MixCodegenGap do
  # Conveniences for MixCodegenGap tasks.
  @moduledoc false
  ...
  ## Examples
      ...
      iex> Mix.MixCodegenGap.inflect("user")
      [alias: "User",
       human: "User",
       base: "MixCodegenGap",
       web_module: "MixCodegenGapWeb",
       module: "MixCodegenGap.User",
       scoped: "User",
       singular: "user",
       path: "user"]

      iex> Mix.MixCodegenGap.inflect("Admin.User")
      [alias: "User",
       human: "User",
       base: "MixCodegenGap",
       web_module: "MixCodegenGapWeb",
       module: "MixCodegenGap.Admin.User",
       scoped: "Admin.User",
       singular: "user",
       path: "admin/user"]

      iex> Mix.MixCodegenGap.inflect("Admin.SuperUser")
      [alias: "SuperUser",
       human: "Super user",
       base: "MixCodegenGap",
       web_module: "MixCodegenGapWeb",
       module: "MixCodegenGap.Admin.SuperUser",
  ...
  def inflect(singular) do
    base       = Mix.MixCodegenGap.base()
    web_module = base |> web_module() |> inspect()
    scoped     = MixCodegenGap.Naming.camelize(singular)
    path       = MixCodegenGap.Naming.underscore(scoped)
    singular   = String.split(path, "/") |> List.last
    module     = Module.concat(base, scoped) |> inspect
    alias      = String.split(module, ".") |> List.last
    human      = MixCodegenGap.Naming.humanize(singular)
  ...
  defp app_base(app) do
    case Application.get_env(app, :namespace, app) do
      ^app -> app |> to_string |> MixCodegenGap.Naming.camelize()
      mod  -> mod |> inspect()
    end
  end
  ...
  and falls back to MixCodegenGap's `priv` directory.
  """
    ...
    case Application.get_env(this_otp_app, :generators)[:context_app] do
      ...
      false ->
        ...
        via cli option:
            ...
            mix codegap.gen.[task] --context-app some_app
